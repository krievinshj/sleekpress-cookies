...
### Fixed
- The textdomain is loaded on `init` rather than on `plugins_loaded`. WordPress
  6.7 warns about the latter, so any site with debugging on had a notice from
  this plugin on every request.
